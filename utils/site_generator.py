...
def convert_markdown_to_html(text):
    """Convert Markdown to HTML, including Obsidian-style links to commentaries."""
        ...
        if os.path.isdir(book_path):
            ...
            for parasha in sorted(os.listdir(book_path)):
                ...
                if os.path.isdir(parasha_path):
                    ...
                    he_text = convert_markdown_to_html(markdown_to_html(read_markdown_file(os.path.join(parasha_path, "HE.md"))))
                    hu_text = convert_markdown_to_html(markdown_to_html(read_markdown_file(os.path.join(parasha_path, "HU.md"))))
                    commentary_html = ""
                    ...
                    if os.path.exists(perusim_path):
                        for commentary_file in os.listdir(perusim_path):
                            if commentary_file.endswith(".md"):
                                ...
                                commentary_html += f'<div id="{file_id}" class="commentary">{markdown_to_html(read_markdown_file(os.path.join(perusim_path, commentary_file)))}</div>'
